...
class Log{
	...
	String pw;
	String name;
	Log(String id, String pw, String name){
		this.id = id;
		this.pw = pw;
		this.name = name;
	}
...
//		System.out.println("<< JDBC 오라클 접속 테스트>>");
	...
	static void checkLogin(String id,String pw) throws Exception{
		boolean check = false;
		String name = "";
		...
		if(check) {
			...
			Class.forName(driver);
			Connection conn = DriverManager.getConnection(url, dbId, dbPw);
			String sql = "UPDATE idconn SET status= 'login' WHERE id= '" + id+"'";
			Statement stmt = conn.createStatement();
	...
	public static void main(String[] args) throws Exception {
		loginCheck();
		if(loginCheck) {
			System.out.println("이미 "+loginName+"님이 로그인중입니다.");
		}else {
			while(true) {
				String id = id();
				String pw = pw();
